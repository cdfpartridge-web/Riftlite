...
import { readMulliganLabPack } from "@/lib/mulligan-lab/server";
...
export async function GET(request: NextRequest) {
  ...
  const initiativeRaw = request.nextUrl.searchParams.get("initiative");
  const initiative = initiativeRaw === "first" || initiativeRaw === "second" ? initiativeRaw : undefined;
  ...
  return NextResponse.json(await readMulliganLabPack({
