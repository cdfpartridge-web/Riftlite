...
import { buildHubStats, formatWeeklyReport, loadHubMatches, requireBotRequest } from "@/lib/discord/bot";
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ hubId: string }> }) {
  ...
  return Response.json({ hubId, text: formatWeeklyReport(stats), stats });
}
